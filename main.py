...
def save_experiment_results(n, k, q, weights, num_points, ilp_time, phase2_time, total_sols, unique_sols):
    """
        ...
        with open(filename, mode='a', newline='', encoding='utf-8') as f:
            ...
            if not file_exists:
                writer.writerow([
                    ...
                    "ILP_Time", "Phase2_Time", "Total_Solutions", "Unique_Solutions"
                ])
            
            writer.writerow([
                datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                n, k, q, str(list(weights)), num_points,
                f"{ilp_time:.4f}", f"{phase2_time:.4f}", total_sols, unique_sols
            ])
...
def run_classification(n, k, q, weights_str, base_code_counts=None, points_km1=None):
    ...
    try:
        ...
        solutions = extender.build_and_solve(points, hyperplanes, base_code_counts, points_km1)
    ...
    if not solutions:
        ...
        save_experiment_results(n, k, q, target_weights, num_points, ilp_time, 0.0, 0, 0)
        return
    ...
    # 결과 파일 저장
    save_experiment_results(n, k, q, target_weights, num_points, ilp_time, phase2_time, len(solutions), len(unique_solutions))
